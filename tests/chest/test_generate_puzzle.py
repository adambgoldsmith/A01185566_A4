...
class TestGeneratePuzzle(TestCase):
    @patch('helper_functions.chest.generate_pattern', side_effect=['XOXIIV'])
    ...
    @patch('helper_functions.chest.generate_pattern', side_effect=['XVOIVX'])
    @patch('random.randint', side_effect=[3])
    def test_generate_puzzle_random_pattern(self, _, __):
        self.assertEqual("I", generate_puzzle())

    @patch('helper_functions.chest.generate_pattern', side_effect=['VVVVVV'])
